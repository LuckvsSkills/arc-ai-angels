...
echo "================================="
echo "OPENCLAW INCIDENT KILL SWITCH V3"
echo "================================="
...
echo "=== SERVICE STATUS BEFORE ==="
{
  ...
  systemctl list-units --type=service --all | grep 'openclaw-agent@' || true
...
echo
echo "=== STOPPING GATEWAY (TERM) ==="
pkill -TERM -x openclaw-gateway || true
sleep 2

echo
echo "=== STOPPING GATEWAY (KILL IF STILL RUNNING) ==="
if pgrep -x openclaw-gateway >/dev/null 2>&1; then
  pkill -KILL -x openclaw-gateway || true
fi
