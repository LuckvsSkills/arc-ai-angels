...
CONTROL_CONFIG_FILE = RUNTIME_DIR / "control" / "control_config.json"
DISPATCH_CANDIDATES_FILE = QUEUE_DIR / "dispatch_candidates.json"
...
def load_control_config() -> dict:
    ...
    return read_json(CONTROL_CONFIG_FILE, default)


def build_dispatch_candidates(execution_requests: List[dict], control_config: dict) -> List[dict]:
    candidates = []

    for request in execution_requests:
        if request.get("status") != "pending_execution":
            continue

        candidate = {
            "dispatch_id": f"dispatch_{safe_slug(request.get('task_id', ''))}",
            ...
            "related_session": request.get("related_session", ""),
            "related_journal": request.get("related_journal", ""),
            "created_at": utcnow().isoformat(),
            "dispatch_status": "candidate",
            "dispatch_mode": control_config.get("mode", "dry_run"),
            "api_enabled": bool(control_config.get("api_enabled", False)),
            "openclaw_enabled": bool(control_config.get("openclaw_enabled", False)),
            "dispatch_enabled": bool(control_config.get("dispatch_enabled", False)),
            "api_calls": 0,
            "execution_decision": "not_dispatched",
            "reason": "Safe mode: dispatch candidates only. No API/OpenClaw call was made."
        }

        candidates.append(candidate)

    write_json(DISPATCH_CANDIDATES_FILE, {
        "generated_at": utcnow().isoformat(),
...
def main() -> int:
    ...
    dispatch_candidates = build_dispatch_candidates(execution_requests, control_config)
    ...
    log(f"Dispatch candidate count: {len(dispatch_candidates)}")
