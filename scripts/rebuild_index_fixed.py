#!/usr/bin/env python3
"""
Arc Control Center - Index Fixed
- Theme panel met goede contrastkleuren
- Intro sectie
...
def generate_sidebar(progress):
    ...
    for chapter in progress.get("chapters", []):
        ...
        chapter_items += f'''
        <div class="nav-item" data-chapter="{chapter_id}" onclick="toggleChapter(this)">
        ...
        for cluster in chapter.get("clusters", []):
            for block in cluster.get("blocks", []):
                ...
                if match:
                    ...
                    chapter_items += f'<li data-chapter="{chapter_id}"><a href="chapters/blocks/{chapter_id}/{block_file}" onclick="event.stopPropagation()">Block {num:02d}: {block_title}</a></li>\n'
        ...
        <div class="theme-panel" id="themePanel">
            ...
            <div class="theme-group">
                ...
                <div class="color-dots">
                    <button class="color-dot active" style="background:#d6b35e" onclick="setPreset('obsidian_gold')" title="Gold"></button>
                    <button class="color-dot" style="background:#36c9ff" onclick="setPreset('graphite_cyan')" title="Cyan"></button>
                    <button class="color-dot" style="background:#9b7cff" onclick="setPreset('midnight_purple')" title="Purple"></button>
                    <button class="color-dot" style="background:#2fd3c5" onclick="setPreset('slate_teal')" title="Teal"></button>
                </div>
            </div>
        </div>
    </div>
