...
os.makedirs('~/arc_ai_angels/backup', exist_ok=True)

# Save current files as backup (if they exist)
try:
    with open(os.path.expanduser('~/arc_ai_angels/website/index.html'), 'r') as f:
        current_index = f.read()
    with open(os.path.expanduser('~/arc_ai_angels/backup/index_backup_latest.html'), 'w') as f:
        f.write(current_index)
    print("💾 Backup gemaakt van huidige index.html")
except:
    print("⚠️  Geen huidige index.html gevonden")
...
# ECHTE OUDE LAYOUT - Zoals het was in het begin
# ZONDER chapter pagina links, alleen de originele structuur

old_layout_html = '''<!DOCTYPE html>
<html lang="nl">
<head>
    <meta charset="UTF-8">
    ...
    <script>
        ...
        async function loadChapters() {
            try {
                ...
                // Render sidebar navigation - ALLEEN hoofdstukken, geen uitklapbare blocks
...
# Write the OLD layout back
with open(os.path.expanduser('~/arc_ai_angels/website/index.html'), 'w') as f:
    f.write(old_layout_html)

print("✅ index.html TERUGGEZET naar originele layout!")
print("🎨 Structuur:")
print("   - Sidebar met Arc AI Angels compact block")
print("   - Hoofdstukken in sidebar (zonder uitklapbare blocks)")
print("   - Hero section 'Arc AI Angels'")
print("   - 2 kolommen: Arc Control Center + Arc AI Angels")
print("   - 4 Mermaid diagrammen")
print("")
print("🔧 Sidebar hoofdstukken zijn nu alleen progress indicators")
print("   (geen uitklapbare blocks meer)")
print("")
print("💾 Backup opgeslagen in: ~/arc_ai_angels/backup/")
print("")
print("Draai: python3 ~/arc_ai_angels/restore_original.py")
