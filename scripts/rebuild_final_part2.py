...
# CHAPTERS INDEX PAGE - Same layout as index.html
        ...
        <main class="flex-1 overflow-y-auto">
            ...
            <section class="relative py-12 px-8 border-b border-white/10">
                <div class="max-w-6xl mx-auto">
                    <h1 class="text-4xl font-bold font-mono glow-text mb-2">Alle Chapters & Blocks</h1>
                    <p class="text-lg text-gray-400">Compleet overzicht van het Arc AI Angels curriculum</p>
                </div>
            </section>

            <!-- Stats -->
            <section class="p-8">
                <div class="max-w-6xl mx-auto grid grid-cols-4 gap-4 mb-8">
                ...
                <!-- Chapters Grid -->
                <div id="chapters-container" class="grid lg:grid-cols-2 gap-6">
                    <!-- Chapters loaded via JS -->
                </div>
            </section>
        </main>
        ...
        async function loadChapters() {
            try {
                ...
                data.chapters.forEach((chapter, idx) => {
                    ...
                    card.className = 'chapter-card-large glass rounded-2xl overflow-hidden';
                    card.innerHTML = `
                        <div class="p-6" style="background: linear-gradient(135deg, ${chapter.color}15 0%, transparent 100%); border-left: 4px solid ${chapter.color};">
                            ...
                            <div class="space-y-2 max-h-64 overflow-y-auto">
                                ${chapter.blocks.map(block => `
                                    <a href="${chapter.id}.html#${block.id}" 
                                       class="flex items-center gap-3 p-3 rounded-lg transition-all hover:translate-x-2 ${block.completed ? 'opacity-60' : ''}"
                                       style="background: ${chapter.color}10; border-left: 3px solid ${block.completed ? '#10b981' : chapter.color};">
                                        ...
                                        ${block.completed ? '<span class="ml-auto text-green-500 text-xs">✓</span>' : '<span class="ml-auto text-xs opacity-50">→</span>'}
                                    </a>
                                `).join('')}
                            </div>
                    ...
                    sidebarItem.innerHTML = `
                        <a href="${chapter.id}.html" class="chapter-header block p-3 rounded-lg mb-1 hover:bg-white/5 transition-all">
                            ...
                            <div class="mt-1 flex items-center gap-2">
                                ...
                                <span class="text-xs text-gray-500">${chapterCompleted}/${chapterTotal}</span>
                            </div>
                        </a>
                    `;
...
# Write chapters/index.html
with open(os.path.expanduser('~/arc_ai_angels/website/chapters/index.html'), 'w') as f:
    f.write(chapters_index_html)

print("✅ chapters/index.html gebouwd!")
print("   - Zelfde layout als index.html (sidebar + main content)")
print("   - Grote chapter cards met alle blocks")
print("   - Stats bovenaan")
print("   - Theme dropdown")
print("")
print("Nu ga ik de individuele chapter pagina's bouwen...")
